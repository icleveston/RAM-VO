...
class Main:
        ...
        with tqdm(total=self.num_train) as pbar:
            ...
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for t in range(self.num_glimpses):
                    ...
                    # Call the model
                    h_state, l_t, b_t, pred, p, _, _, entropy = self.model(x_0, x_1, l_t, h_state)
                    
                    # Get the prediction for each glimpse
                    predicted_all.append(denormalize_displacement(pred, 100))
                ...
                predicted_all = torch.stack(predicted_all).transpose(1, 0)
                ...
                R = (1/(1 + torch.square(torch.sub(predicted.detach(), y.detach())).mean(dim=1)))
                
                # Compute the reward
                R = R.unsqueeze(1).repeat(1, self.num_glimpses)
                
                # Compute losses for differentiable modules
                loss_action = loss_mse(predicted, y)
                ...
                # Get the mae
                mae = loss_l1(predicted.detach(), y)
            ...
            train_data = (mae_array, reward_array, loss_reinforce_array, loss_baseline_array)
    ...
    def _validate(self, epoch):
        ...
        validation_data = (mae_array, reward_array, loss_reinforce_array, loss_baseline_array)
    ...
    def test(self, model_name):
        ...
        for i, (x, y) in enumerate(self.test_loader):
            ...
            if i == 1:
                ...
                trans = transforms.Compose([
                    NormalizeInverse([0.0004], [0.0200]),
        ...
        # Denormalize gt
        std_inv = 1 / (33.90598 + 1e-8)
        mean_inv = -0.0037339155 * 33.90598
        
        y_array = (y_array - mean_inv) / std_inv
        predicted_array = (predicted_array - mean_inv) / std_inv
        
        # Get samples every 20 frames
        skip = len(y_array) // 20
                         
        # Save the first prediction
        samples = [[h.cpu().numpy(), p.cpu().numpy()] for h, p in zip(y_array[::skip], predicted_array[::skip])]         
