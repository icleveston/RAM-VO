...
from dataset import KittiDatasetOriginal
from utils import *
...
def get_data_loader(batch_size, dataset, train_seq, val_seq, test_seq,
    ...
    trans_input_images = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.4209265411], [0.2889825404])
    ])
    
    # Compose the transformations
    trans_input_op = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.3373341858, 0.3594599962], [0.2222646028, 0.2552885711])
    ])
    ...
    if train_seq is not None:
        dataset_train = KittiDatasetOriginal(sequences=train_seq, trans_input_images=trans_input_images, trans_input_op=trans_input_op, trans_output=trans_output, preload=preload)
    ...
    if val_seq is not None:
        dataset_val = KittiDatasetOriginal(sequences=val_seq, trans_input_images=trans_input_images, trans_input_op=trans_input_op, trans_output=trans_output, preload=preload)
    ...
    if test_seq is not None:
        dataset_test = KittiDatasetOriginal(sequences=[test_seq], trans_input_images=trans_input_images, trans_input_op=trans_input_op, trans_output=trans_output, preload=preload, should_skip=False)
