...
def create_cluttered_translated_mnist():
    (X_train, y_train),(X_test, y_test) = get_cluttered_translated_mnist(4, 60, 60, 8, 8)
...
if not os.path.isfile("logs/cluttered-translated-mnist-60.json"):
    logger = JSONLogger(path="logs/cluttered-translated-mnist-60.json")
    bayes_opt.subscribe(Events.OPTMIZATION_STEP, logger)
else:
    load_logs(bayes_opt, logs=["logs/cluttered-translated-mnist-60.json"])
    logger = JSONLogger(path="logs/cluttered-translated-mnist-60.json")
    bayes_opt.subscribe(Events.OPTMIZATION_STEP, logger)
